...
const STATUS_STYLES: Record<string, { color: string; icon: React.ReactNode }> = {
  ...
  archived:  { color: 'rgba(var(--ink),0.3)', icon: <Archive   size={10} /> },
    ...
    <div style={{ maxWidth: '960px' }}>
      ...
      {rows.length === 0 ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px', padding: '60px', textAlign: 'center' }}>
          <FileText size={32} style={{ color: 'rgba(var(--ink),0.15)', marginBottom: '12px' }} />
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'rgba(var(--ink),0.3)', letterSpacing: '0.15em', textTransform: 'uppercase', margin: '0 0 16px' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 90px 70px 80px', gap: '0', padding: '10px 20px', borderBottom: '1px solid var(--border)', background: 'rgba(var(--ink),0.02)' }}>
            {['Article', 'Type', 'Status', 'Views', 'Actions'].map(h => (
              <span key={h} style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(var(--ink),0.3)' }}>
                {h}
              </span>
            ))}
          </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ minWidth: 0, paddingRight: '16px' }}>
                  ...
                  <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(var(--ink),0.35)', margin: 0, letterSpacing: '0.04em' }}>
                ...
                {/* Type */}
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(var(--ink),0.4)' }}>
                ...
                {/* Views */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'rgba(var(--ink),0.35)' }}>
                  <Eye size={10} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', marginTop: '24px' }}>
          {page > 1 && (
            <Link href={`/admin/articles?status=${status}&page=${page - 1}${search ? `&q=${encodeURIComponent(search)}` : ''}`}
              style={{ padding: '6px 14px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '5px', fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(var(--ink),0.6)', textDecoration: 'none', letterSpacing: '0.08em' }}>
              ← Prev
            </Link>
          )}
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(var(--ink),0.3)', letterSpacing: '0.1em', padding: '0 8px' }}>
            {page} / {totalPages}
          </span>
          {page < totalPages && (
            <Link href={`/admin/articles?status=${status}&page=${page + 1}${search ? `&q=${encodeURIComponent(search)}` : ''}`}
