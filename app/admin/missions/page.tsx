...
const STATUS_STYLES: Record<string, { color: string; icon: React.ReactNode }> = {
  ...
  completed:        { color: 'rgba(255,255,255,0.35)', icon: <CheckCircle size={10} /> },
    ...
    <div style={{ maxWidth: '960px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <form action="/admin/missions" method="GET" style={{ display: 'flex', gap: '6px' }}>
          ...
          />
          <button type="submit" style={{ padding: '7px 14px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '6px', color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }}>
            Search
          </button>
        </form>
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', marginTop: '24px' }}>
          {page > 1 && (
            <Link href={`/admin/missions?status=${status}&page=${page - 1}${search ? `&q=${encodeURIComponent(search)}` : ''}`}
              style={{ padding: '6px 14px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '5px', fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(255,255,255,0.6)', textDecoration: 'none' }}>
              ← Prev
            </Link>
          )}
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(255,255,255,0.3)', padding: '0 8px' }}>
            {page} / {totalPages}
          </span>
          {page < totalPages && (
            <Link href={`/admin/missions?status=${status}&page=${page + 1}${search ? `&q=${encodeURIComponent(search)}` : ''}`}
