...
const STATUSES: { value: MissionStatus; label: string; color: string }[] = [
  ...
  { value: 'completed',      label: 'Completed',      color: 'rgba(var(--ink),0.4)' },
        ...
        <div>
          ...
          {form.timeline.length === 0 ? (
            <div style={{ padding: '24px', background: 'var(--surface)', border: '1px dashed var(--border)', borderRadius: '8px', textAlign: 'center' }}>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(var(--ink),0.3)', letterSpacing: '0.12em', textTransform: 'uppercase', margin: 0 }}>
              ...
              {form.timeline.map((event, i) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                    ...
                    />
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.15em', textTransform: 'uppercase', color: event.completed ? 'var(--green)' : 'rgba(var(--ink),0.3)', flex: 1 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', position: 'sticky', top: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
            {STATUSES.map(s => (
              ...
              >
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.1em', textTransform: 'uppercase', color: form.status === s.value ? 'var(--white)' : 'rgba(var(--ink),0.45)' }}>
                  {s.label}
                </span>
                {form.status === s.value && (
        ...
        <SidePanel label="Mission Type">
          <div style={{ position: 'relative' }}>
            ...
            <ChevronDown size={12} style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', color: 'rgba(var(--ink),0.4)', pointerEvents: 'none' }} />
          </div>
        </SidePanel>
...
function FieldLabel({ children, hint }: { children: React.ReactNode; hint?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '6px' }}>
      ...
      {hint && <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'rgba(var(--ink),0.3)', letterSpacing: '0.04em' }}>{hint}</span>}
    </div>
  )
}
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden' }}>
      <div style={{ padding: '8px 14px', borderBottom: '1px solid var(--border)', background: 'rgba(var(--ink),0.02)' }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(var(--ink),0.4)' }}>{label}</span>
      </div>
...
const iconBtnStyle: React.CSSProperties = {
  ...
  color: 'rgba(var(--ink),0.4)', cursor: 'pointer',
}
