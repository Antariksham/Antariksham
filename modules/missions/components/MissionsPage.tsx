...
export function MissionsPage({ missions, featured, total }: Props) {
  ...
  // Lead card: first featured mission (or first mission if none featured)
  const lead = featured.length > 0 ? featured[0] : missions[0] || null

  // Grid: all missions when no filter, filtered missions when filter active
  const gridItems = activeStatus
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {STATUSES.map(s => (
              <FilterBtn
                ...
                color={s.value ? STATUS_COLOR[s.value] : '#4f8ef7'}
              >
                {s.label}
              </FilterBtn>
            ))}
          ...
          <a href={`/missions/${lead.slug}`} style={{ display: 'block', textDecoration: 'none', marginBottom: 'clamp(24px,4vw,40px)' }}>
            ...
            >
              ...
              {lead.featuredImage ? (
                ...
                <div style={{ width: '100%', height: '280px', background: 'linear-gradient(135deg, #12121a 0%, #1a1a2e 50%, #0a0a0f 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ padding: 'clamp(20px,3vw,36px)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
                  ...
                  {lead.agency && (
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.45)' }}>
                      {lead.agency.shortName}
                    </span>
                  )}
                  {lead.destination && (
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(255,255,255,0.3)', letterSpacing: '0.05em' }}>
                      → {lead.destination}
                    </span>
                  )}
                </div>

                <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(24px,3.5vw,40px)', fontWeight: 400, color: '#ffffff', lineHeight: 1.12, margin: '0 0 14px', letterSpacing: '-0.01em', maxWidth: '800px' }}>
                  {lead.name}
                </h2>
...
function MissionCard({ mission }: { mission: MissionCard }) {
  return (
    <a href={`/missions/${mission.slug}`} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
      <div
        style={{ background: '#12121a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column', cursor: 'pointer', transition: 'border-color 0.2s, transform 0.2s' }}
      ...
      >
        ...
        {mission.featuredImage ? (
          ...
          <div style={{ width: '100%', height: '100px', background: 'linear-gradient(135deg, #12121a 0%, #1a1a2e 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            {mission.agency && (
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#4f8ef7' }}>
          ...
          {/* Name */}
          <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(18px,1.8vw,22px)', fontWeight: 400, color: '#ffffff', lineHeight: 1.25, margin: 0, flex: 1 }}>
            {mission.name}
          </h3>
