...
export function DeepSpaceTracker({ initialProbes, updatedAt }: Props) {
  ...
  useEffect(() => {
    try {
      setDisplayDate(new Date(lastUpdated).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }))
    } catch (_e) {
      setDisplayDate(lastUpdated)
    }
  }, [lastUpdated])
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // Sanitize — only keep known fields to prevent runtime crashes
      if (Array.isArray(data.probes) && data.probes.length > 0) {
        const safe = data.probes.map((p: Record<string, unknown>) => ({
          id:                  String(p.id || ''),
          name:                String(p.name || ''),
          agency:              String(p.agency || 'NASA'),
          launchDate:          String(p.launchDate || '1977-01-01'),
          distanceFromEarth:   Number(p.distanceFromEarth) || 0,
          distanceFromSun:     Number(p.distanceFromSun) || 0,
          velocity:            Number(p.velocity) || 0,
          signalDelay:         Number(p.signalDelay) || 0,
          missionPhase:        String(p.missionPhase || ''),
          targetBody:          String(p.targetBody || ''),
      ...
      if (data.updatedAt) setLastUpdated(String(data.updatedAt))
    } catch (_e) {
  ...
  useEffect(() => {
    const id = setInterval(refresh, 3600000)
