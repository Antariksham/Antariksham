...
export function DeepSpaceTracker({ initialProbes, updatedAt }: Props) {
  ...
  // Minimum valid distances per probe — if API returns below these, use current state
  ...
  // Auto-refresh once on mount to get live data, then every 30 minutes
  useEffect(() => {
    const timer = setTimeout(() => { refresh() }, 800)  // slight delay so page renders first
