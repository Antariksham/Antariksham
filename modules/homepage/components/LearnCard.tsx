...
export function LearnCard({ topic, lang = DEFAULT_LANGUAGE }: { topic: Topic; lang?: LanguageCode }) {
  ...
  return (
    <Link href={sectionHref('learn', topic.slug, lang)} className="card">
      ...
      <div className="card-body">
        ...
        <h3 className="card-title" lang={lang}>{topic.title}</h3>
        {topic.excerpt && <p className="card-excerpt" lang={lang}>{topic.excerpt}</p>}
      </div>
    </Link>
  )
}
